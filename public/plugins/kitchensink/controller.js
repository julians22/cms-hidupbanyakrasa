  ...
  function addAccessors($scope) {
    ...
    $scope.addIText = function() {
      ...
      var textSample = new fabric.IText(text.slice(0, getRandomInt(0, text.length)), {
        left: 50,//getRandomInt(350, 400),
        top: 50,//getRandomInt(350, 400),
        fontFamily: 'helvetica',
        angle: getRandomInt(-10, 10),
        fill: '#' + getRandomColor(),
        scaleX: 0.5,
    ...
    function addImage(imageName, minScale, maxScale) {
      ...
      fabric.Image.fromURL(base_url+'requirement/images/' + imageName, function(image) {
      ...
      $scope.addSticker = function(value) {
        addImage(value, 0.75, 1);
      };

      $scope.addStickerT = function(value) {
        addImage(value, 0.1, 0.25);
      };    
